...
export function generateSummary(run: RunState, allEvents: ChronicleEvent[]): string[] {
  ...
  // FIX: Use a type guard to correctly type the filtered events, allowing safe access to 'intentId'.
  ...
  // FIX: Use a type guard with `.find()` to correctly type the found event, allowing safe access to 'name'.
